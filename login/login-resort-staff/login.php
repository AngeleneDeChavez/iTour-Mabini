...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Query to fetch the user data, including the status and login-related fields
    $sql = "SELECT id, password, status, failed_attempts, lockout_time FROM resorts WHERE username='$input_username'";
        ...
        if ($row['failed_attempts'] >= 3 && $current_time < $lockout_time) {
            ...
            if ($row['status'] == 'inactive') {
                ...
                if (password_verify($input_password, $row['password'])) {
                    ...
                    $stmt = $conn->prepare("UPDATE resorts SET failed_attempts = 0, lockout_time = NULL WHERE id = ?");
                    $stmt->bind_param("i", $row['id']);
                    $stmt->execute();

                    // Set session and redirect
                    $_SESSION['user_id'] = $row['id'];
                    header("Location: ../../RESORT/index.php");
                    exit();
                } else {
                    ...
                    $stmt = $conn->prepare("UPDATE resorts SET failed_attempts = ?, lockout_time = ? WHERE id = ?");
